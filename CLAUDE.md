...
## מצב נוכחי בתפריט העליון / כפתור טעינת קבצים (⚠️ לא סופי, יש חוסר ודאות)
...
> וב-`pages/dashboard/quarterly-targets.js`. **הצביעה בצהוב טרם בוצעה** - הסגנון `styles.uploadBtn` עדיין לבן
> (`background: '#fff'`, `border: '1px solid #d1d5db'`) בשני הקבצים.
...
## עוד לעשות / פתוח
- לצבוע צהוב את כפתור "טעינת קבצים" בשני הדשבורדים (המיקום כבר אומת - ראה סעיף למעלה).
