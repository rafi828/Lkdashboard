      ...
      {rawRows.length > 0 && (
        <>
          ...
          <div style={{ ...styles.card, display: 'flex', gap: 14, flexWrap: 'wrap', alignItems: 'flex-end' }}>
            ...
            <button onClick={() => handleExport(agent)} style={styles.exportBtn}>ייצוא לשליחת מייל (Word Mail Merge)</button>
          </div>
...
function handleExport(agent) {
  ...
  window.location.href = `/api/dashboard/quarterly-targets-export?${params.toString()}`;
}
